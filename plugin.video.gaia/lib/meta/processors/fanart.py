...
class MetaFanart(object):
	...
	@classmethod
	def movie(self, idImdb = None, idTmdb = None, cache = False):
		try:
			if idImdb or idTmdb:
				id = []
				if idImdb: id.append(idImdb)
				if idTmdb: id.append(idTmdb)
	...
	@classmethod
	def show(self, idTvdb = None, season = None, cache = False):
		try:
			if idTvdb:
				data = self.retrieve(type = MetaFanart.TypeShow, id = idTvdb, cache = cache)
				if data is False: return False
